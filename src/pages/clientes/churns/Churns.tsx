 ...
 * Dados vêm de ./mockData.ts; trocar por API depois não muda a UI.
 */
...
function TipBox({ tip, children }: { tip: TipState; children: React.ReactNode }) {
  // Posiciona à direita do cursor; se estiver na metade direita, joga pra esquerda
...
const MOTIVO_BADGE: Record<string, string> = {
  'Resultado inexistente': 'border-red-500/40 bg-red-500/10 text-red-300',
...
function TabelaChurns() {
  const [busca, setBusca] = useState('')
  const linhas = useMemo(() => {
    const q = busca.trim().toLowerCase()
    if (!q) return churnsClientes
    return churnsClientes.filter(
            ...
            {linhas.map((c) => (
              <tr key={c.id} className="border-b border-border/60 last:border-b-0 transition-colors hover:bg-bg-soft/50">
                <td className="px-4 py-3">
                  ...
                  <p className="text-[10px] text-muted">{c.nicho}</p>
                </td>
