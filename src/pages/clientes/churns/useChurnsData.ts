...
export function useChurnsData(): ChurnsData {
  ...
  const data = useMemo<Omit<ChurnsData, 'loading' | 'reload'>>(() => {
    ...
    const ativos = clientes.filter((c) => c.status !== 'churn' && !c.arquivado_em)
    ...
    const churnedRaw = clientes.filter((c) => c.status === 'churn')
    const lista: ClienteChurn[] = churnedRaw.map((c) => {
    ...
    lista.sort((a, b) => (b.dataChurn || '').localeCompare(a.dataChurn || ''))
  ...
  return { loading: false, ...data, reload: load }
}
